...
- **PR #2314** by @WanderWang (closes #2305, refs #749) — WebUI-created profiles now match CLI skill seeding behavior. Pre-fix, creating a new profile via the WebUI without cloning left the `skills/` directory empty, while CLI-created profiles received bundled skills via `seed_profile_skills()`. The fix adds the missing `seed_profile_skills(profile_path, quiet=True)` call in `api/profiles.py:create_profile_api()` when `clone_from is None`, with `ImportError` caught separately for Docker/standalone environments without `hermes_cli`, and a generic `Exception` fallback ensuring seed failures remain non-fatal. Three behavioural regression tests cover fresh-profile seeding, cloned-profile skip, and graceful seed-failure handling.
